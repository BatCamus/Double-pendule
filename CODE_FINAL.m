...
if Ener_ODE45&&ERR_ODE_45
           ...
           Ec2=0.5*m2*((l1^2)*(x(:,2).^2)+(l2^2)*(x(:,4).^2)+(2*l1*l2).*(cos(x(:,1)-x(:,3)).*x(:,2).*x(:,4))); %Energie cinÃƒÂ©tique pendule 2
...
if Ener_Newmark && Ener_ODE45==0
           ...
           Ec2=0.5*m2*((l1^2)*(dxt(:,1).^2)+(l2^2)*(dxt(:,2).^2)+(2*l1*l2).*(cos(xt(:,1)-xt(:,2)).*dxt(:,1).*dxt(:,2))); %Energie cinÃƒÂ©tique pendule 2
...
%             caxis('auto')
            ...
            title('Erreur relative sur theta1 en % pour le schema de newmark par rapport à la solution analytique');
...
%             caxis('auto')
            xlabel('Theta1 en degres');
            ylabel('Theta2 en degres');
            title('Erreur relative sur theta2 en % pour le schema de newmark par rapport à la solution analytique');
            colorbar('EastOutside')
            drawnow
    end
end
...
if GrilleEner||GrilleTr
    ...
    dtheta=0.5;                         % Pas d'angle           
    Range=180;                           % Angles extremes Ã  atteindre
